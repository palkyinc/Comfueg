...
class Ciudad
{
    ...
    public function __construct($dato = null)
    {
      if (is_array($dato)) {
          $this->setDatosPorArray($dato);
                ...
                $this->obtenerCiudadPorId($dato);
              }
    }

    public function obtenerCiudadPorId($datoId)
    {
    ...
    public static function getCollectionCiudad ($query, $paginas) {
      ...
      $stmt->execute();
      if ($stmt->rowCount() >= 1) {
            return $stmt->fetchAll(PDO::FETCH_CLASS, 'Ciudad');
        }
      return false;
    }
