...
class Nivel
{
    ...
    public function __construct($dato = null)
    {
      if (is_array($dato)) {
          $this->setDatosPorArray($dato);
                ...
                $this->obtenerNivelPorId($dato);
              }
    }

    public function obtenerNivelPorId($datoId)
    {
    ...
    public static function getCollectionNivel ($query, $paginas) {
      ...
      $stmt->execute();
      if ($stmt->rowCount() >= 1) {
            return $stmt->fetchAll(PDO::FETCH_CLASS, 'Calle');
        }
      return false;
    }
    public function guardarEnDb(){
      ...
      if($stmt->rowCount() === 1) {
        $resultado ['item_auditable'] = 'calles';
        return $resultado;
      } else return false;
    }
    private function obternerDiferencias () {
      $datoDB = new Calle ($this->getId());
